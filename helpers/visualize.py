...
def save_scannet_eval_format_to_mesh(scene_id, 
    ...
    os.makedirs(join(data_dir, 'vis_mesh', scene_id), exist_ok=True)
    config = res_dir.split('/')[-1]
    save_path = join(data_dir, 'vis_mesh', scene_id, f'{scene_id}_{config}.ply')
...
if __name__ == '__main__':
    ...
    data_dir = "/data/ScanNet"
    res_dir = "/data/ScanNet/results"
    
    target_folders = ["default"]
    

    for folder_name in target_folders:

        final_dir = join(res_dir, folder_name)
        if not os.path.exists(final_dir):
            continue
        save_scannet_eval_format_to_mesh(scene_id, final_dir, data_dir)