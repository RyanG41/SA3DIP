...
def main():
    ...
    # all_scenes = ['scene0011_00.txt', 'scene0011_01.txt', 'scene0015_00.txt']
    all_scenes = sorted(val_split)
    for j in range(len(all_scenes)):
        pred_files = [all_scenes[j]]
        ...
        output_file = os.path.join('results/newgt_perscene/perscene_mix0.04', all_scenes[j])
        # evaluate
        evaluate(pred_files, gt_files, opt.pred_path, output_file)
