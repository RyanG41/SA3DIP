...
class ScanNet_SA3DIP(SA3DIPBase):
        ...
        if seg_ids is None:
            if points_obj_labels_path is None:
                ...
                if not self.scannetpp:
                    scene_seg_path = \
                        join(base_dir, 'scans', scene_id, f'{scene_id}_vh_clean_2.{KThresh}000.{colorWeight}0000.segs.json')
                else:
                    scene_seg_path = \
...
def everything_seg(args):
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    agent = ScanNet_SA3DIP(points, args)
    ...
    with CodeTimer('Assign instance labels', dict_collect=time_collection):
        labels_fine_global = agent.assign_label(points, 
                                                ...
                                                KThresh=args.KThresh,
                                                colorWeight=args.colorWeight)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--KThresh', type=str, default=None)
    parser.add_argument('--colorWeight', type=str, default=None)
